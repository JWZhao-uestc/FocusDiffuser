...
class Resize(object):
    ...
    def __init__(self, size, label_size):
    ...
    def __call__(self, image, label):
        ...
        label = label.resize(self.label_size[::-1],resample=Image.NEAREST)
...
class RandomMask(object):
    def __init__(self, p=0.5, random_ratio=0.3, iterations=3):
        self.p = p
    ...
    def __call__(self, image, label):
        ...
        for _ in range(self.iterations):
            if random.random() < self.p:
                #generator = MaskingGenerator(input_size=(h,w),
                                                    #num_masking_patches=int(h*w*self.random_ratio), 
                                                    #min_num_patches=16, )
                #mask = generator()
                #image = image*np.expand_dims(mask, 2)
                h_start,w_start = random.randint(0,h), random.randint(0,w)
                height, width = random.randint(0,int(h*self.random_ratio)), random.randint(0,int(w*self.random_ratio))
                image[h_start:h_start+height, w_start:w_start+width, :] = 0
...
if __name__ == '__main__':
    ...
    img = cv2.imread('/mnt/ZJW/Research_code/COD/diffusion-segmenter-COD/DATA/train/Imgs/camourflage_00001.jpg')
    mask = cv2.imread('/mnt/ZJW/Research_code/COD/diffusion-segmenter-COD/DATA/train/GT/camourflage_00001.png')

    rand_ = RandomMask()
    image, label = rand_(img, mask)
    cv2.imwrite('flip_mask.png', image)
