...
class Resize(object):
    ...
    def __init__(self, image_size, gt_size):
        assert (isinstance(image_size, collections.Iterable) and len(image_size) == 2)
        self.image_size = image_size
        self.gt_size = gt_size

    def __call__(self, image, label, edge):
        image = cv2.resize(image, self.image_size[::-1], interpolation=cv2.INTER_LINEAR)
        #label = cv2.resize(label, self.gt_size[::-1], interpolation=cv2.INTER_NEAREST)
        label = Image.fromarray(label.astype(np.uint8))
        label = label.resize(self.gt_size[::-1],resample=Image.NEAREST)
        label = np.array(label)
        
        #edge = cv2.resize(edge, self.gt_size[::-1], interpolation=cv2.INTER_NEAREST)
        edge = Image.fromarray(edge.astype(np.uint8))
        edge = edge.resize(self.gt_size[::-1],resample=Image.NEAREST)
        edge = np.array(edge)
